...
from sklearn.metrics import roc_curve, roc_auc_score, precision_score, recall_score, f1_score, accuracy_score, classification_report, confusion_matrix, precision_recall_curve
...
for name, config in models.items():
    ...
    try:
        ...
        opt_threshold, test_f1, test_precision, test_recall = find_optimal_threshold(y_test, y_pred_proba)
        # Calculate accuracy using optimal threshold
        test_accuracy = accuracy_score(y_test, (y_pred_proba > opt_threshold).astype(int))
    except Exception:
        test_auc = np.nan
    ...
    roc_curves_data[name] = {"fpr": fpr, "tpr": tpr, "auc": test_auc, "recall": test_recall, "precision": test_precision, "f1": test_f1, "accuracy": test_accuracy, "optimal_threshold": opt_threshold}

    print(f"Best parameters: {random_search.best_params_}")
    print(f"CV AUC: {cv_score:.4f}")
    print(f"Test AUC: {test_auc:.4f}")
    if 'opt_threshold' in locals():
...
# Find optimal threshold for F1 score
opt_threshold_voting, voting_f1, voting_precision, voting_recall = find_optimal_threshold(y_test, y_pred_voting)
# Calculate accuracy using optimal threshold
voting_accuracy = accuracy_score(y_test, (y_pred_voting > opt_threshold_voting).astype(int))

roc_curves_data["Voting Ensemble"] = {"fpr": fpr_voting, "tpr": tpr_voting, "auc": voting_auc, "recall": voting_recall, "precision": voting_precision, "f1": voting_f1, "accuracy": voting_accuracy, "optimal_threshold": opt_threshold_voting}
...
print(f"Voting Ensemble Optimal Threshold: {opt_threshold_voting:.4f}")
...
# Find optimal threshold for F1 score
opt_threshold_stacking, stacking_f1, stacking_precision, stacking_recall = find_optimal_threshold(y_test, y_pred_stacking)
# Calculate accuracy using optimal threshold
stacking_accuracy = accuracy_score(y_test, (y_pred_stacking > opt_threshold_stacking).astype(int))

roc_curves_data["Stacking Ensemble"] = {"fpr": fpr_stacking, "tpr": tpr_stacking, "auc": stacking_auc, "recall": stacking_recall, "precision": stacking_precision, "f1": stacking_f1, "accuracy": stacking_accuracy, "optimal_threshold": opt_threshold_stacking}
...
print(f"Stacking Ensemble Optimal Threshold: {opt_threshold_stacking:.4f}")
...
results_df = pd.DataFrame({
    ...
    'Optimal Threshold': [data.get('optimal_threshold', np.nan) for data in roc_curves_data.values()],
    'Accuracy': [data.get('accuracy', np.nan) for data in roc_curves_data.values()],
...
numeric_columns = ['Test AUC', 'Optimal Threshold', 'Accuracy', 'Recall', 'Precision', 'F1', 'CV AUC']
for col in numeric_columns:
    if col in results_df.columns:
        results_df[col] = results_df[col].apply(lambda x: round(x, 4) if pd.notna(x) and isinstance(x, (int, float)) and x != 'N/A' else x)
