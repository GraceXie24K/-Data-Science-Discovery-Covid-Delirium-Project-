...
N_FEATURES_TO_KEEP = 300   # Number of important featuresq to keep during feature selection
...
serial_annotation_df = load_table('serial_samples_annotation.csv')
...
# Note: Missing value imputation moved to after correlated column removal
...
# Handle missing values (moved here after correlated column removal)
# Update impute column lists to only include columns that still exist
cat_impute_cols = [c for c in cat_impute_cols if c in combined_df.columns]
num_impute_cols = [c for c in num_impute_cols if c in combined_df.columns]

if ENABLE_IMPUTE:
    print("Missing value imputation enabled")
    ...
    if num_impute_cols:  # Only process if there are numeric columns to impute
        missing_percentage = combined_df[num_impute_cols].isna().sum() / len(combined_df)
        cols_to_drop = missing_percentage[missing_percentage > 0.2].index.tolist()

        if cols_to_drop:
            print(f"Dropping {len(cols_to_drop)} columns with >10% missing values")
    ...
    print("Missing value imputation disabled - dropping all columns with NaN values")
    # Drop all columns with any missing values when imputation is disabled
    cols_with_nans = combined_df.columns[combined_df.isna().any()].tolist()
    # Remove target column from the list if it's there
    cols_with_nans = [col for col in cols_with_nans if col != target]
    
    if cols_with_nans:
        print(f"Dropping {len(cols_with_nans)} columns with any NaN values: {cols_with_nans}")
        combined_df = combined_df.drop(columns=cols_with_nans)
        print("All NaN-containing columns dropped")
    else:
...
for model_name, model_config in models.items():
    ...
    # Store metrics
    f1_scores[model_name] = opt_f1
    precision_scores[model_name] = opt_precision
    recall_scores[model_name] = opt_recall
    accuracy_scores[model_name] = opt_accuracy
    optimal_thresholds[model_name] = opt_threshold
    
    print(f"Test AUC: {test_auc:.4f}")
    print(f"Optimal Threshold: {opt_threshold:.4f}")
    print(f"Accuracy: {opt_accuracy:.4f}, Recall: {opt_recall:.4f}, Precision: {opt_precision:.4f}, F1: {opt_f1:.4f}")
...
voting_fpr, voting_tpr, _ = roc_curve(y_test, y_pred_voting)

# Find optimal threshold for Voting Ensemble
voting_opt_threshold, voting_opt_f1, voting_opt_precision, voting_opt_recall = find_optimal_threshold(y_test, y_pred_voting)
voting_y_pred_opt = (y_pred_voting >= voting_opt_threshold).astype(int)
voting_opt_accuracy = accuracy_score(y_test, voting_y_pred_opt)

# Store Voting Ensemble metrics
f1_scores['Voting Ensemble'] = voting_opt_f1
precision_scores['Voting Ensemble'] = voting_opt_precision
recall_scores['Voting Ensemble'] = voting_opt_recall
accuracy_scores['Voting Ensemble'] = voting_opt_accuracy
optimal_thresholds['Voting Ensemble'] = voting_opt_threshold

print(f"Voting Ensemble AUC: {voting_auc:.4f}")
print(f"Voting Ensemble Optimal Threshold: {voting_opt_threshold:.4f}")
print(f"Voting Ensemble Accuracy: {voting_opt_accuracy:.4f}, Recall: {voting_opt_recall:.4f}, Precision: {voting_opt_precision:.4f}, F1: {voting_opt_f1:.4f}")
...
stacking_fpr, stacking_tpr, _ = roc_curve(y_test, y_pred_stacking)

# Find optimal threshold for Stacking Ensemble
stacking_opt_threshold, stacking_opt_f1, stacking_opt_precision, stacking_opt_recall = find_optimal_threshold(y_test, y_pred_stacking)
stacking_y_pred_opt = (y_pred_stacking >= stacking_opt_threshold).astype(int)
stacking_opt_accuracy = accuracy_score(y_test, stacking_y_pred_opt)

# Store Stacking Ensemble metrics
f1_scores['Stacking Ensemble'] = stacking_opt_f1
precision_scores['Stacking Ensemble'] = stacking_opt_precision
recall_scores['Stacking Ensemble'] = stacking_opt_recall
accuracy_scores['Stacking Ensemble'] = stacking_opt_accuracy
optimal_thresholds['Stacking Ensemble'] = stacking_opt_threshold

print(f"Stacking Ensemble AUC: {stacking_auc:.4f}")
print(f"Stacking Ensemble Optimal Threshold: {stacking_opt_threshold:.4f}")
print(f"Stacking Ensemble Accuracy: {stacking_opt_accuracy:.4f}, Recall: {stacking_opt_recall:.4f}, Precision: {stacking_opt_precision:.4f}, F1: {stacking_opt_f1:.4f}")

# Store ensemble results
results['Voting Ensemble'] = {'model': voting_clf, 'test_auc': voting_auc, 'cv_auc': 'N/A'}
results['Stacking Ensemble'] = {'model': stacking_clf, 'test_auc': stacking_auc, 'cv_auc': 'N/A'}

# Store ROC curve data for ensemble models
roc_curves_data['Voting Ensemble'] = {"fpr": voting_fpr, "tpr": voting_tpr, "auc": voting_auc}
roc_curves_data['Stacking Ensemble'] = {"fpr": stacking_fpr, "tpr": stacking_tpr, "auc": stacking_auc}
...
results_df = pd.DataFrame([
    {
        ...
        'Optimal Threshold': optimal_thresholds.get(name, np.nan),
        'Accuracy': accuracy_scores.get(name, np.nan),
        'Recall': recall_scores.get(name, np.nan),
        'Precision': precision_scores.get(name, np.nan),
        'F1 Score': f1_scores.get(name, np.nan)
    }
    for name in results.keys()
])

# Round numeric columns to 4 decimal places
numeric_columns = ['Test AUC', 'CV AUC', 'Optimal Threshold', 'Accuracy', 'Recall', 'Precision', 'F1 Score']
...
# Save results
results_df.to_csv('serialGraph/model_performance_results.csv', index=False)
print(f"\nSerial transcriptomic analysis complete!")
print(f"Results saved to: serialGraph/model_performance_results.csv")
