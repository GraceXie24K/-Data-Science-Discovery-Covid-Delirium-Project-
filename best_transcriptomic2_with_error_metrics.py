...
from sklearn.metrics import roc_curve, roc_auc_score, precision_score, recall_score, f1_score, accuracy_score, precision_recall_curve, classification_report, confusion_matrix
...
# Helper function to find optimal threshold that maximizes F1 score
def find_optimal_threshold(y_true, y_pred_proba):
    """Find the threshold that maximizes F1 score using ROC curve (AUC curve)."""
    # Use roc_curve to get thresholds from the ROC curve
    fpr, tpr, thresholds_roc = roc_curve(y_true, y_pred_proba)
    
    # Note: thresholds_roc has one less element than fpr/tpr
    # The last element of fpr/tpr corresponds to threshold such that no samples are predicted as positive
    # We'll use the thresholds from ROC curve and calculate F1, precision, recall for each
    ...
    # Calculate F1 score, precision, and recall for each threshold from ROC curve
    for i in range(len(thresholds_roc)):
        threshold = thresholds_roc[i]
    ...
    # Also check threshold=0.5 explicitly
    ...
    # Add 0.5 threshold to the list if it's not already there (or very close)
    threshold_05_exists = any(np.abs(np.array(valid_thresholds) - 0.5) < 0.01)
    if not threshold_05_exists:
        f1_scores_list.append(f1_05)
        valid_thresholds.append(0.5)
        valid_precisions.append(prec_05)
        valid_recalls.append(rec_05)
    ...
    if len(f1_scores_list) > 0:
        ...
        # Fallback: use default 0.5 threshold
        best_threshold = 0.5
        best_f1 = f1_05
        best_precision = prec_05
        best_recall = rec_05
    
    # Additional optimization: if recall is too high (>=0.99) and F1 is low,
    # prefer a threshold that gives better balance
    if best_recall >= 0.99 and best_f1 < 0.7:
        # Strategy 1: Look for thresholds with recall between 0.7-0.95 that have good F1
        best_balanced_idx = None
        best_balanced_f1 = 0
        
        for i in range(len(f1_scores_list)):
            if 0.7 <= valid_recalls[i] <= 0.95 and f1_scores_list[i] > best_balanced_f1:
                best_balanced_f1 = f1_scores_list[i]
                best_balanced_idx = i
        
        # Strategy 2: If no good balanced threshold found, use Youden's J statistic
        # Youden's J = TPR - FPR, which maximizes the difference between true positives and false positives
        # This corresponds to finding the point on ROC curve farthest from the diagonal
        if best_balanced_idx is None:
            best_youden = -1
            best_balanced_idx = None
            for i in range(len(thresholds_roc)):
                # Calculate Youden's J: TPR - FPR
                youden_j = tpr[i] - fpr[i]
                if valid_recalls[i] < 0.99 and youden_j > best_youden:
                    best_youden = youden_j
                    best_balanced_idx = i
                    best_balanced_f1 = f1_scores_list[i]
        
        # If we found a better balanced threshold, use it
        # Prefer balanced threshold if it has at least 80% of max F1 and avoids recall=1.0
        if best_balanced_idx is not None and best_balanced_f1 > best_f1 * 0.8:
            best_idx = best_balanced_idx
            best_threshold = valid_thresholds[best_idx]
            best_f1 = f1_scores_list[best_idx]
            best_precision = valid_precisions[best_idx]
            best_recall = valid_recalls[best_idx]
...
roc_curves_data = {}
cv_scores = {}
f1_scores = {}  # Store F1 scores for all models
precision_scores = {}  # Store precision scores for all models
recall_scores = {}  # Store recall scores for all models
accuracy_scores = {}  # Store accuracy scores for all models
optimal_thresholds = {}  # Store optimal thresholds for all models
...
for name, config in models.items():
    ...
    f1_scores[name] = opt_f1
    precision_scores[name] = opt_precision
    recall_scores[name] = opt_recall
    accuracy_scores[name] = opt_accuracy
    ...
    print(f"Test F1 (optimized): {opt_f1:.4f}")
...
y_pred_voting_proba = voting_classifier.predict_proba(X_test_scaled)[:, 1]
voting_auc = roc_auc_score(y_test, y_pred_voting_proba)
fpr_voting, tpr_voting, _ = roc_curve(y_test, y_pred_voting_proba)

# Find optimal threshold for F1 score
opt_threshold_voting, opt_f1_voting, opt_precision_voting, opt_recall_voting = find_optimal_threshold(y_test, y_pred_voting_proba)
y_pred_voting_opt = (y_pred_voting_proba >= opt_threshold_voting).astype(int)

# Calculate accuracy using optimal threshold
opt_accuracy_voting = accuracy_score(y_test, y_pred_voting_opt)

roc_curves_data["Voting Ensemble"] = {"fpr": fpr_voting, "tpr": tpr_voting, "auc": voting_auc}
f1_scores["Voting Ensemble"] = opt_f1_voting
precision_scores["Voting Ensemble"] = opt_precision_voting
recall_scores["Voting Ensemble"] = opt_recall_voting
accuracy_scores["Voting Ensemble"] = opt_accuracy_voting
optimal_thresholds["Voting Ensemble"] = opt_threshold_voting
...
print(f"Optimal Threshold: {opt_threshold_voting:.4f}")
print(f"Voting Ensemble F1 (optimized): {opt_f1_voting:.4f}")
print(f"Voting Ensemble Precision: {opt_precision_voting:.4f}")
print(f"Voting Ensemble Recall: {opt_recall_voting:.4f}")

# Stacking Classifier
# Use only individual models (exclude ensemble models) as base models
base_models = [(name, model) for name, model in trained_models.items() if 'Ensemble' not in name]
...
y_pred_stacking_proba = stacking_classifier.predict_proba(X_test_scaled)[:, 1]
stacking_auc = roc_auc_score(y_test, y_pred_stacking_proba)
fpr_stacking, tpr_stacking, _ = roc_curve(y_test, y_pred_stacking_proba)

# Find optimal threshold for F1 score
opt_threshold_stacking, opt_f1_stacking, opt_precision_stacking, opt_recall_stacking = find_optimal_threshold(y_test, y_pred_stacking_proba)
y_pred_stacking_opt = (y_pred_stacking_proba >= opt_threshold_stacking).astype(int)

# Calculate accuracy using optimal threshold
opt_accuracy_stacking = accuracy_score(y_test, y_pred_stacking_opt)

roc_curves_data["Stacking Ensemble"] = {"fpr": fpr_stacking, "tpr": tpr_stacking, "auc": stacking_auc}
f1_scores["Stacking Ensemble"] = opt_f1_stacking
precision_scores["Stacking Ensemble"] = opt_precision_stacking
recall_scores["Stacking Ensemble"] = opt_recall_stacking
accuracy_scores["Stacking Ensemble"] = opt_accuracy_stacking
optimal_thresholds["Stacking Ensemble"] = opt_threshold_stacking
...
print(f"Optimal Threshold: {opt_threshold_stacking:.4f}")
print(f"Stacking Ensemble F1 (optimized): {opt_f1_stacking:.4f}")
print(f"Stacking Ensemble Precision: {opt_precision_stacking:.4f}")
print(f"Stacking Ensemble Recall: {opt_recall_stacking:.4f}")
...
results_df = pd.DataFrame({
    ...
    'Optimal Threshold': [optimal_thresholds.get(name, np.nan) for name in roc_curves_data.keys()],
    'Accuracy': [accuracy_scores.get(name, np.nan) for name in roc_curves_data.keys()],
    'F1 Score': [f1_scores.get(name, np.nan) for name in roc_curves_data.keys()],
    'Precision': [precision_scores.get(name, np.nan) for name in roc_curves_data.keys()],
    'Recall': [recall_scores.get(name, np.nan) for name in roc_curves_data.keys()]
})

results_df = results_df.sort_values('Test AUC', ascending=False)

# Round numeric columns to 4 decimal places
numeric_columns = ['Test AUC', 'CV AUC', 'Optimal Threshold', 'Accuracy', 'F1 Score', 'Precision', 'Recall']
for col in numeric_columns:
    if col in results_df.columns:
        results_df[col] = results_df[col].apply(lambda x: round(x, 4) if pd.notna(x) and isinstance(x, (int, float)) and str(x) != 'N/A' else x)
...
# F1 scores summary (now included in model_performance_results.csv)
print(f"\n=== F1 SCORES SUMMARY (Optimized Thresholds) ===")
print(f"\n📊 NOTE: F1 scores are often lower than AUC scores because:")
print(f"   • AUC measures ranking ability (can the model distinguish positives from negatives?)")
print(f"   • F1 measures classification performance at a specific threshold (precision × recall balance)")
print(f"   • A model can have high AUC (good ranking) but lower F1 if precision/recall are imbalanced")
print(f"   • This is normal and expected - AUC and F1 measure different aspects of model performance")
print(f"   • All metrics (including F1, accuracy, precision, recall) are saved to: graph/model_performance_results.csv")

print("Comprehensive visualizations saved to 'graph' folder:")
...
print("- graph/model_performance_results.csv (comprehensive metrics: Test AUC, CV AUC, Optimal Threshold, Accuracy, F1 Score, Precision, Recall)")